...
// simulationはheadlessで回せることが前提なので、依存の先までEbitengineを含まない。
//
...
func TestDependenciesDoNotIncludeEbitengine(t *testing.T) {
	...
	queue := []string{modulePath + "/internal/simulation"}
	for len(queue) > 0 {
		pkgPath := queue[0]
		queue = queue[1:]
		if visited[pkgPath] {
		...
		for _, imported := range productionImports(t, dir) {
			if strings.Contains(imported, "ebiten") {
				t.Errorf("%s が %s をimportしている（simulationの依存に入っている）", pkgPath, imported)
				continue
			}
