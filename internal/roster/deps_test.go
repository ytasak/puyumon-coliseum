...
// roster layerはゲーム定義のデータなので、依存の先までEbitengineを含まない。
//
// rosterはsession・Bot・UIのどこからも読まれる土台で、ここがEbitengineへ
// 届くと上の層の分離がまとめて崩れる。このpackage自身のimportだけでは
// 不十分で、battleを経由して引き込まれていないことまで確かめる。
func TestDependenciesDoNotIncludeEbitengine(t *testing.T) {
	...
	queue := []string{modulePath + "/internal/roster"}
	for len(queue) > 0 {
		pkgPath := queue[0]
		queue = queue[1:]
		if visited[pkgPath] {
		...
		for _, imported := range productionImports(t, dir) {
			if strings.Contains(imported, "ebiten") {
				t.Errorf("%s が %s をimportしている（roster layerの依存に入っている）", pkgPath, imported)
				continue
			}
