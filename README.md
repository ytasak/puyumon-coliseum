...
- 下部: 1 つの枠付き窓。対戦の経過と選択肢がこの中に入る。最初は出す 1 体、
  対戦中は「たたかう」/「こうたい」、技は 4 つと残り PP
...
`internal/bot` は「どの Action を選ぶか」だけを返す層で、対戦のルールも session の進行も持たない。
session の内部ではなく外側の actor なので、production code は `internal/battle` だけに依存し、
...
| 画面の配色 | 4 階調のグリーングレーだけで描く | 1996 年の携帯機 RPG の見た目へ寄せる。**階調が 4 つしかないので色相で意味を分けられない。** 残り HP・押せない選択肢・戦闘不能は、明度に加えて縞・斜線・× でも区別する。参照するのは階調の少なさという視覚文法だけで、原作の画像・ロゴ・UI は使わない |
| コマンドの見せ方 | 1 つの枠付き窓を細線で仕切る | 独立したボタンを並べると初代の画面から離れる。**描画と当たり判定は `commandGrid` の 1 か所から作る。** 別々に計算すると見えているものと押せるものがずれる |
| Composite Emoji の階調 | offscreen へ描いてから Kage shader で 4 階調へ丸める | フルカラーのままだと UI から浮く。1 枚にまとめてから通すので、キャラクターと particle が重なったところも同じ段へ揃う。Ebitengine 同梱の shader なので dependency は増えない |
...
| 相手を倒した turn | 追加効果は起きない。ただし**吸収・自爆・多段・Struggle の反動は起きる** | ROM は相手が倒れた時点で処理を終えるが、この 4 つだけは「倒しても最後まで処理する」側に入っている（Struggle の反動は ROM の AlwaysHappenSideEffects） |
...
| Struggle の持ち方 | 固定 move set へ足さず、Battle Engine 側の synthetic move として持つ | roster のデータに現れないので UI の技欄にも出ない。キャラクターごとの分岐も増えない |
| Struggle の行動表現 | 専用の `StruggleAction`。`MoveAction` の特別な slot にしない | 「技枠を使わない行動」を型で表せる。PP を消費するかどうかの分岐も 1 箇所で済む |
| Struggle の Event | `MoveUsed` を再利用し、`Slot` は `NoMoveSlot`（-1） | 専用 Event を足さずに UI から判別できる。`Move` が `struggle` であることでも分かる |
| 反動の表現 | `EffectRecoil` として 1/2 を固定で持つ。率を data の parameter にしない | 採用技に他の反動技が無い。同じく 1/2 固定の吸収（`EffectDrain`）と形を揃えた |
| 反動と戦闘不能の順序 | 相手への damage → 反動 damage → 相手の `Fainted` → 使用者の `Fainted` | turn の出来事をそのままの順で読める。相打ちのときも「どちらが先に倒れたか」ではなく同じ Action の結果として並ぶ |
